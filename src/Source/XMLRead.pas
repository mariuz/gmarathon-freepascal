{
    $Id: XMLRead.pas,v 1.1 2005/04/26 13:11:47 rjmills Exp $
...
//Removed to clear up compiler warning.  RJM
{var
...
begin
  if CheckFor('<?') then begin
    ...
    if AnsiUpperCase(StrPas(checkbuf)) = 'XML' then
...
begin
  ...
  try
    ReadXMLFile(ADoc, TStream(stream), AFilename);
  finally
    stream.Free;
  end;
end;
...
begin
  ...
  stream := TFileStream.Create(AFilename, fmOpenRead);
  try
    ReadDTDFile(ADoc, TStream(stream), AFilename);
	finally
    stream.Free;
  end;
end;
