...
begin
	...
	begin
		// Set cursor position
		CaretPoint.Line := FExecutelineBegin;
		CaretPoint.Char := 1;
		CaretXY := CaretPoint;
	end;
	Refresh;
						...
						begin
							...
							begin
								...
								begin
									...
									FListPos.X := Pt.X + ((Length(FBuffer) - 1) * Canvas.TextWidth('W'));
									FListPos.Y := Pt.Y + Canvas.TextHeight('W');
					...
					if FDotLBX.ItemIndex<>-1 then
						begin
							...
							begin
								...
								BlockBegin:=BufferCoord(idx+1,CaretY);
								BC:=BlockBegin;
								...
								if Idx > 0 then
									begin
										CaretXY:=BufferCoord(BC.Char+Idx,bc.Line);
										Key:=#0;
									end;
							end;
						end;
...
(*  //Rmoved by RJM. to clean up a compiler hint...
...
begin
					...
					begin
						...
						begin
							...
							begin
								...
								Pt := Self.ClientToScreen(Pt);
								FSQL.Left := Pt.X;
								FSQL.Top := Pt.Y + Canvas.TextHeight('W');
								;
	...
	begin
		...
		begin
			...
			l:=PixelsToRowColumn(pt.X, pt.Y);
			tokenType:=-1;
			start:=-1;
			ID:='';
						
			if Assigned(FGetHyperlinkToken) then
				FGetHyperlinkToken(self,point(l.Column, l.Row),ID,start,tokenType)
			else
				GetHighlighterTokenAtRowCol(point(l.Column, l.Row),ID,start,tokenType);
...
begin
	GetHighlighterTokenAtRowCol(Point(CaretX, CaretY),ID,start,tokenType);
...
begin
	...
	begin
		...
		if FLinking then
		begin
      l := PixelsToRowColumn(mpos.x, mpos.y);
			if fDecorator.DecorationAt(Point(l.Column, l.Row))<>fHyperLink then
				begin
...
begin
	result:=CoorToIndex(Point(BlockBegin.Char, BlockBegin.Line));
end;
...
var
   pt : tpoint;
begin
  pt := IndexToCoor(value);
	BlockBegin:= BufferCoord(pt.x, pt.y); 
end;

function TSyntaxMemoWithStuff2.GetSelLength: integer;
begin
	result:=CoorToIndex(Point(BlockEnd.Char, Blockend.Line))-CoorToIndex(Point(BlockBegin.Char, BlockBegin.Line));
end;
...
var
   pt : TPoint;
begin
  pt := IndexToCoor(CoorToIndex(Point(BlockBegin.Char, BlockBegin.Line))+Value);
	BlockEnd:= BufferCoord(pt.x, pt.Y);
end;
...
begin
	...
	begin
		...
		try
			...
			if BlockBegin.Char < (LeftChar - 1) then
				LeftChar := BlockBegin.Char
			else if BlockEnd.Char >= (CharsInWindow + LeftChar) then
				LeftChar := BlockEnd.Char - CharsInWindow + 2;

			// Make sure Y is Visible
			if BlockBegin.Line < (TopLine - 1) then
				TopLine := BlockBegin.Line + 1
			else if BlockEnd.Line > (TopLine + (LinesInWindow - 2)) then
				TopLine := BlockEnd.Line - (LinesInWindow - 2);
		finally
			DecPaintLock;
		end;
	end;
...
begin
  ...
  bc:=CaretXY;
	temp:=GetTokenAtRowCol(point(bc.Char, bc.Line),start,stop);

	pt.X:=start;
  pt.Y := bc.line;
	allow:=TRUE;
	if assigned(fChangeKeywordCase) then
		fChangeKeywordCase(self,pt,temp,allow);

	if allow and (temp<>'') and Highlighter.isKeyword(temp) then
		begin
			temp:=AnsiUpperCase(temp);
			line:=lines[CaretY-1];
			lines[CaretY-1]:=copy(line,1,start-1)+temp+copy(line,stop,length(line));
		end;
end;
