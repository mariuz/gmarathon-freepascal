...
{                                                                  }
{******************************************************************}
// $Id: LexLib.pas,v 1.3 2005/04/13 16:04:29 rjmills Exp $
...
type
    TCustomLexer = class (TComponent)
      public
        yyDebug : Boolean;
        yyinput, yyoutput : TStringList;     (* input and output file            *)
        yyerrorfile       : TStringList;        (* standard error file              *)
...
var
  i : Integer;
begin
  if (bufptr = 0) and not (yyLineNo > yyInput.Count - 1) then
  begin
...
begin
  ...
  if c=nl then
    yyoutput.Text := yyoutput.Text + ''
  else
    yyoutput.Text := yyoutput.Text + c;
end;
...
constructor TCustomLexer.Create(anOwner: TComponent);
begin
  inherited create( anOwner);
  yyinput := TStringList.Create;
  yyoutput := TStringList.Create;
  yyerrorfile := TStringList.Create;
  yylineno := 0;
  yyclear;
end;
