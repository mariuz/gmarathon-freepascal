...
interface
uses windows, SynEdit, SynEditTypes, classes, graphics, sysutils;
...
type
  TDecoratorPlugin = class(TSynEditPlugin)
    ...
    procedure AfterPaint(Canvas: TCanvas; Const AClip: TRect; FirstLine, LastLine: integer); override;
          ...
          with fDecorations[i] do
            if (Line>=FirstLine) and (Line<=LastLine) then
              begin
                p.Char:=StartIndex;
                p.Line:=Line;
                l:=BufferToDisplayPos(p);
                l.Row:=Line+1;
                pt:=RowColumnToPixels(l);
                r.left:=pt.X;
                r.bottom:=pt.Y;
...
procedure TDecoratorPlugin.AfterPaint(Canvas: TCanvas; Const AClip: TRect; FirstLine,LastLine: integer);
begin
