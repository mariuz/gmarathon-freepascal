...
uses {$IFDEF FPC} {$IFDEF WINDOWS}Windows,{$ENDIF} LCLIntf, LCLType, LMessages, Messages, ibase60dyn, {$ELSE} Windows, Messages, {$ENDIF} SysUtils, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, ComCtrls, ExtCtrls, DB, Printers, Menus, ClipBrd, IBQuery, SynEdit, SyntaxMemoWithStuff2, BaseDocumentDataAwareForm, MarathonInternalInterfaces, FrameMetadata, FrameDescription, rmCompatControls;
    ...
    try
      try
        qryDomain.Edit;
				...
				TIBTransaction(qryDomain.Transaction).Commit;
				SetObjectName(edColumn.Text);
			except
				on E : Exception do
				begin
					TIBTransaction(qryDomain.Transaction).Rollback;
					raise;
				end;
			end;
		finally
...
begin
	...
	begin
		...
		TmpIntf := Self;
		FCompile := TfrmCompileDBObject.CreateCompile(Self, TmpIntf, TIBDatabase(qryDomain.Database), TIBTransaction(qryDomain.Transaction), ctDomain, FCompileText);
