...
- [ ] **Partial indices in extracted DDL** — Firebird 5 added indices with a
  `WHERE` condition, and the 6.0.0 test server has `RDB$INDICES.RDB$CONDITION_SOURCE`.
  `DDLExtractor` does not emit it, so extracting a partial index produces DDL
  that creates a *full* one — a wrong script rather than a missing feature.
  Found by probing the server while looking for what else is new.

- [ ] **Backup history** — `RDB$BACKUP_HISTORY` exists on the test server and
  records nbackup runs. The Maintenance dialog could show it beside the backup
  and restore it already performs.
