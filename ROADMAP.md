...
- [ ] **`SQL SECURITY` on procedures, functions and triggers** — done for tables; the other three object types still neither read nor emit it.
- [ ] **Packages (FB3)** — `RDB$PACKAGES` is not surfaced anywhere: no tree node, no DDL extraction of package headers or bodies, and the bulk wizard cannot export them. Packaged *functions* are now correctly excluded from the standalone list, which makes their absence visible rather than misleading, but the packages themselves remain unsupported.
