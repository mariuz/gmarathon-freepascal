...
**Data grid edits** — the table editor's Data tab runs with IBX `CachedUpdates`, so edits are held. `TfrmTables.PendingDataChanges` renders them through `src/Common/RowEdits.pas`; `ApplyDataChanges` writes them, `CancelDataChanges` drops them. Leaving the Data tab asks about the *pending row changes*, not about the transaction — `CheckCommit`. The grid writes through a `TIBUpdateSQL` built from the table's columns and key when the tab opens; a plain `TIBQuery` over `select *` is read-only, so before that every edit failed with "Dataset is read-only". Tables without a primary key stay read-only deliberately. **Known limit:** a pending *deletion* is invisible to both `UpdatesPending` and a dataset walk, so the preview shows inserts and updates but says nothing about deletions; Apply still writes them. Also note `form_load_test` installs an `Application.OnException` sink - without it an unhandled exception opens a dialog that hangs under Xvfb instead of failing. RowEdits refuses to write an UPDATE or DELETE for a table with no primary key rather than emit one that would match every row that looks alike.
